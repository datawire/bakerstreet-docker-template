...
def configure_for_docker(**kwargs):
    ...
    routable_address = kwargs.get('routable_address', resolve(hosts_file='/etc/hosts', hostname='dockerhost'))
    template_vars = {'routable_address': routable_address,
                     'mapped_port': os.environ['mapped_port'],
                     'dw_directory_host': os.environ['dw_directory_host']}
